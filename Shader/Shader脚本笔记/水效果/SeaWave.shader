...
Shader "Water/SeaWave" {
	Properties {
		...
		_BumpTex ("BumpTex", 2D) = "bump" {} //法线贴图（凹凸贴图）
		...
		_WaveRange ("WaveRange", float) = 0.3  //海波范围
	...
	SubShader {
		...
		void vert (inout appdata_full v, out Input i) {
			...
			i.proj = ComputeScreenPos(UnityObjectToClipPos(v.vertex));//投影
			COMPUTE_EYEDEPTH(i.proj.z);//深度计算
		}
		...
		void surf (Input IN, inout SurfaceOutput o) {
			...
			fixed4 noiseColor = tex2D(_NoiseTex, IN.uv_NoiseTex);//噪音纹理
			...
			o.Normal = UnpackNormal(bumpColor).xyz;//法线纹理
