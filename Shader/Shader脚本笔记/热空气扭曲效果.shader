...
Shader "Xffect/my_distortion" {
	 ...
	 Category {
	  ...
	  Cull Off Lighting Off ZWrite Off  // 关闭剔除，关闭灯光，不记录深度
		  ...
		  SubShader {
			   ...
			   Pass {
					...
					v2f vert (appdata_t v)
					{
						 ...
						 o.vertex = mul(UNITY_MATRIX_MVP, v.vertex);	 // 从模型坐标-世界坐标-视坐标-（视觉平截体乘以投影矩阵并进行透视除法）-剪裁坐标
						
						 #if UNITY_UV_STARTS_AT_TOP  
						  // Direct3D类似平台scale为-1；OpenGL类似平台为1。
						 float scale = -1.0;
						 #else
						 float scale = 1.0;
						 #endif
					...
					half4 frag( v2f i ) : COLOR
					{
						 ...
						 half4 areaCol = tex2D(_AreaTex, i.uvmain);// 对区域图进行采样。
