Shader "YueChuan/ReplaceLightmap/ReplaceRimLightDiffuse"
 {
	Properties 
	{
   ...
   // #include "HDREncode.cginc"
    ...
    VS_OUTPUT MainVS(appdata_full input)
    {
		  ...
		  if (_IsRimLight)
		  {
			  ...
			  output.v3RimColor = pow((1.0 - dot(vEyeDir, v3WNormal)), 1) * _RimColor * _Attenuation;
		  }
	...
	fixed3 DecodeLogLuv(in fixed4 vLogLuv)
	{
     ...
     fixed4 MainPS(VS_OUTPUT input) : COLOR
     {
		  ...
		  return  baseColor * decodeLightmapColor + fixed4(input.v3RimColor,1.0);
     }
